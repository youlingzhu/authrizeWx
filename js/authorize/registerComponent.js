function Component_list() {
    ...
    this.owner = function (data, componentNextPro) {
        ...
        this.three_syndromes_components(data, "是否为受益所有人", arr, dom, function (value) {
            ...
            //  console.log('选中是否是受益人', data.value);
            //  console.log('选中受益人信息上面的第一个赋值', wechartJson[0].is_beneficiary);
    ...
    */
    ...
    this.biz_sub_appid = function (data) {
        ...
        if (subject_boolean) {
            div_hr.style.marginBottom = '-15px';
    ...
    this.input_box_id_card = function (data, dom, componentNextPro) { // componentNextPro 是一个函数；
        ...
        if (data.caption == '线下场所对应的商家APPID(选填)' || data.caption == '小程序截图' || data.caption == '公众号页面截图' || data.caption == '小程序截图' || data.caption == '网站授权函(选填)' || data.caption == '互联网网站对应的商家APPID(选填)') {
            iOne.style.display = 'none';
        }
        ...
        input.onblur = function () {
            ...
            data.value = input.value ? input.value : "";
            //   // console.log('开户名称',this)
            var a = data.verification_method();
            if (a) {
                if (data.caption.slice(0, 5) == '优惠费率活动') {
                    if (data.value) {
                        // console.log(data.value)
                        p.style.display = 'block';
                        p.innerHTML = a;
