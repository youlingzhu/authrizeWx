	...
	if(wechartJson[0].business_scenario[0]) {
		for(var i = 0; i < wechartJson[0].business_scenario.length; i++) {
			...
			if(wechartJson[0].business_scenario.indexOf(str_xiaochengxu) != -1) {   // 小程序场景如果被选中就执行他的input
				yingyongchangjing(div_applets_scene, 'input_public',arr_isnext);
			...
			if(wechartJson[0].business_scenario.indexOf(str_app) != -1) {           //app场景如果被选中就执行他的input
				yingyongchangjing(div_app_scence, 'input_public',arr_isnext);
			...
			if(wechartJson[0].business_scenario.indexOf(str_weixin) != -1) {          // 企业微信如果被选中就执行他的input
				...
				var input_qiye_pic = getClassName_doms(div_enterprise_wechat, 'input_updata');
...
function verification_three() {
	...
	if(discount_rate_id_input.value) {
		// console.log(discount_rate_id_input);
		discount_rate_id_input.onblur();
		if(discount_rate_id_input.hasAttribute('isnext')) {
			arr_isnext.push(discount_rate_id_input);
		}
	}

	if(activities_rate_id_input.value) {
		activities_rate_id_input.onblur();
		if(activities_rate_id_input.hasAttribute('isnext')) {
			arr_isnext.push(activities_rate_id_input);
		}
	}
